...
# Check out the data structure
  str(NH11)
# Take a subset of variables - specified by task: age_p (Age), r_maritl (Marital Status), everwrk (Ever Worked)
# Demographic variables plus wkdayr (Number of work loss days, past 12 months)
...
There are still many missing values. Clean up race/ethnicity then perform mice imputation. We are also done with **wkdayr** so it can go.
...
Replace **everwrk** with the imputed values.

```{r data5, echo=TRUE, eval=TRUE, warning=FALSE}
  NH11_sub$everwrk <- imputed$everwrk
```

Check why summary lists "other" under **r_maritl**.
...
We need to remove "Unknown marital status" as it is not meaningful when modelling. We also re-leveled the **r_maritl** variable to approximate the usual chronology of status, otherwise coefficients may not be sensible (*Under 14 Years* and *Married - spouse in household unknown* did not have any records).
...
Ok, it's all ready to go, we can change **everwrk** to classifications of 1 or 0 and and move on.
...
There seems to be slight multicollinearity between age and marital status, which makes sense since older people are more likely to have journeyed through the chronologically ordered marital statuses. None of the x variables have a strong correlation with the y variable.
...
Including age did not reduce the AIC by much but it did reduce the significance of some of the variables as predictors. So we'll stay with the model which excludes age.
...
This model actually did not predict any probabilities above 0.5 so the accuracy is unchanged from the baseline model. So does the model have any way to distinguish between "ever having worked" or not?
...
Let's look at the ROC Curve to see what threshold value 